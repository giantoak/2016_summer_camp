...
out_list = []
count = 0
with open('negative_sample_full_docs_v2.jl') as f:
    for line in f:
        try:
            data = ujson.loads(line)
            source=data['hits']['hits'][0]['_source']
            out={}
            out['doc_id']=data['hits']['hits'][0]['_id']
            out['content'] = source['extracted_text']
            if source.has_key('extractions'):
                if source['extractions'].has_key('phonenumber'):
                    #out['rate_info'] = data['extractions']['rate']
                    out['phone_1'] = source['extractions']['phonenumber']['results'][0]
            ...
            break
false_df = pandas.DataFrame(out_list)
false_df['group'] = 'train_false'
false_df.to_csv('false_positives_with_phones_text.csv', index=False, sep='\t', encoding='utf-8')
...
eval_df['group'] = 'test'
df = pandas.concat([true_df, false_df, eval_df])
del eval_df, true_df, false_df
...
train_indices = df['group'].apply(lambda x: 'train' in x)
train_phones = df.loc[train_indices, 'phone_1'].tolist()
true_train_phones = df.loc[df['group']=='train_true','phone_1'].tolist()
...
aggregated_group = df.groupby('phone_1')['group'].apply(lambda x: (x == 'train_true').max())
...
save_df.to_csv('model_fit.csv')
df.to_csv('imputation.csv', index=False)  


df['bad'] = df['group']=='train_true'
#X = out[:,2:]
#y = pandas.Series(out[:,1]).astype('int')
y=df[df['group']!='test'].groupby('phone_1').apply(lambda x: (x['group'] == 'train_true').max()).astype('int')    
pipeline.fit(df[df['group'] != 'test'], y)
#rf = RandomForestClassifier(n_jobs=-1, n_estimators=40, random_state=2, oob_score=True)
#rf.fit(out, y_)
probs = pipeline.predict_proba(df[df['group'] != 'test'])
cPickle.dump(pipeline,open('giantoak_ht_model.pkl','wb'))
